...
class UserServiceTest {
    ...
    void deleteUser_success_deletesUser() {
        ...
        when(timeOffRequestRepository.findByUserId(2)).thenReturn(List.of());

        userService.deleteUser(2, 1); // currentUserId=1, deleting id=2

        verify(timeOffRequestRepository).deleteAll(anyList());
        verify(userRepository).delete(sampleUser);
    }

    @Test
    void deleteUser_notFound_throwsIllegalArgument() {
        when(userRepository.findById(99)).thenReturn(Optional.empty());
        assertThrows(IllegalArgumentException.class, () -> userService.deleteUser(99, 1));
    }

    @Test
    void deleteUser_selfDelete_throwsIllegalState() {
        when(userRepository.findById(1)).thenReturn(Optional.of(sampleUser));
        assertThrows(IllegalStateException.class, () -> userService.deleteUser(1, 1));
    }

    @Test
    void deleteUser_pendingRequests_throwsIllegalState() {
        TimeOffRequest pending = new TimeOffRequest();
        pending.setStatus("PENDING");
        when(userRepository.findById(2)).thenReturn(Optional.of(sampleUser));
        when(timeOffRequestRepository.findByUserId(2)).thenReturn(List.of(pending));

        assertThrows(IllegalStateException.class, () -> userService.deleteUser(2, 1));
        verify(userRepository, never()).delete(any());
    }

    @Test
    void deleteUser_onlyApprovedRequests_deletesSuccessfully() {
        TimeOffRequest approved = new TimeOffRequest();
        approved.setStatus("APPROVED");
        when(userRepository.findById(2)).thenReturn(Optional.of(sampleUser));
        when(timeOffRequestRepository.findByUserId(2)).thenReturn(List.of(approved));
